...
export default function TodoListPage() {
  ...
  return (
    <Box sx={{ minHeight: '100vh', bgcolor: 'white' }}>
      ...
      <Paper elevation={0} sx={{ position: 'fixed', top: 0, left: 0, right: 0, height: '80px', zIndex: 1100, bgcolor: 'white', borderBottom: '1px solid #eee', px: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <Stack direction="row" spacing={2} alignItems="center">
          <Avatar sx={{ width: 56, height: 56, bgcolor: 'transparent', border: '2px solid black', color: 'black' }}><PersonIcon sx={{ fontSize: 32 }} /></Avatar>
          {/* AQUI A LOGO É USADA (sem alterações no JSX, pois o nome da variável é o mesmo) */}
          <Box><Box sx={{ bgcolor: '#bbdefb', px: 1, borderRadius: 1, width: 'fit-content' }}><Box component="img" src={logoImg} alt="Logo" sx={{ height: 60, width: 'auto', mb: 0.5, borderRadius: 1 }} /></Box></Box>
        </Stack>

        <Stack direction="row" spacing={2}>
          <Tooltip title="Dashboard"><IconButton onClick={handleGoToStats} sx={actionBtnStyle}><BarChartIcon color="primary" fontSize="large" /></IconButton></Tooltip>
          <Tooltip title="Criar nova tarefa"><IconButton onClick={handleGoToNewPage} sx={actionBtnStyle}><FolderOpenIcon sx={{ color: '#00bcd4' }} fontSize="large" /></IconButton></Tooltip>
          <Tooltip title="Concluir Selecionados"><IconButton onClick={() => showNotification('Use a página de detalhes para concluir', 'info')} sx={{ ...actionBtnStyle, borderColor: '#4caf50' }}><CheckBoxIcon color="success" fontSize="large" /></IconButton></Tooltip>
          <Tooltip title="Apagar Selecionados"><IconButton onClick={handleDeleteSelected} sx={{ ...actionBtnStyle, borderColor: '#ef5350' }}><DeleteForeverIcon color="error" fontSize="large" /></IconButton></Tooltip>
        </Stack>
      </Paper>
